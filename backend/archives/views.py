        ...
        if slack_archive.testzip() is None:
            ...
            if 'users.json' in archive_files and 'channels.json' in archive_files:
                ...
                for user in users_list:
                    slack_user_serializer = SlackUserUploadSerializer(data={'user_data': user})
                ...
                for channel in channels_list:
                    channel_serializer = ChannelUploadSerializer(data={'channel_data': channel})
                ...
                for fileinfo in slack_archive.infolist():
                    ...
                    if fileinfo.file_size > 0:
                        ...
                        if len(path) > 1:
                            channel_name = path[0]
                            channel = Channel.objects.get(archive=archive, name=channel_name)
                            print (path, "channel: ", channel)        ##
                            messages_file = slack_archive.open(fileinfo)
                            messages_list = json.load(messages_file)
                            messages_file.close()
                            for msg in messages_list:
                                print ("msg: ", msg)    ##
                                message_serializer = MessageUploadSerializer(data={'message_data': msg})
